...
# last change on the 1 October 2019 
# to make sure that the function working even without xvar
# (Tim Cole sussestion)
# change MS Saturday, August 14, 2010
...
################################################################################
# function 2 
  get.intervals <- function (xvar, xcut.points, from, to) 
  {
    ...
    {stop(paste("The specified `xcut.points' are not within the range of the x: (", min(xvar),
                " , ", max(xvar), ")"))}
    min_xvar <-  if (!missing(from))  from else  min(xvar) 
    max_xvar <-  if (!missing(to))    to else  max(xvar)  
    int <- c(min_xvar, xcut.points, max_xvar)
    ii <- 1:(length(int)-1)
    r <- 2:length(int)
    x1 <- int[ii]
    xr <- int[r]
...
if(is.null(xcut.points)) 
  ...
  { # if xcut.points is set
    g.in <- get.intervals(xvar, xcut.points, from, to) 
  }
  howmany <- dim(g.in)[1]
    fname <- obj$family[1]
     qfun <- paste("q",fname,sep="")
...
obj1$tau.fv <- tau1
      main1 <- main[i]
  sampleval <- centiles(obj1, xvar=xvar1, cent, legend=legend, main=main1, main.gsub = main.gsub, 
